...
page = st.sidebar.selectbox(
    "Select Page",
    ["📋 View Cases", "➕ Add New Case", "✏️ Edit Case", "🗑️ Delete Case", "📊 Dashboard"],
...
if page == "📋 View Cases":
    ...
    if df.empty:
        ...
        if case_id:
            try:
                ...
                if not specific_case_df.empty:
                    ...
                    st.warning(f"⚠️ Case #{case_id} not found. Showing all cases.")
            except ValueError:
        ...
        display_df = display_df.drop(['resolved', 'row_index'], axis=1)
        ...
        # Show unresolved cases with action buttons
        unresolved_df = filtered_df[filtered_df['resolved'] == False]
        if not unresolved_df.empty:st.subheader("🚨 Unresolved Cases - Action Required")
        ...
        for idx, case in unresolved_df.iterrows():
            ...
            st.success("✅ All cases have been resolved!")
            # st.balloons()

        # Show case details for selected rows
